...
\setbeamertemplate{footline}{%
  \hbox{\begin{beamercolorbox}[wd=\paperwidth,ht=2.6ex,dp=1.6ex,leftskip=1.5em,rightskip=1.5em]{}%
    ...
    \hfill{\color{arMuted}\scriptsize\bfseries\insertframenumber\,/\,14}%
  \end{beamercolorbox}}%
}
...
\begin{frame}
  ...
  \begin{tikzpicture}[
    ...
    \node[core] (graph)  at (6.7,3.0) {LangGraph workflow};
    \node[box]  (policy) at (6.7,1.6) {LoRA routing policy};
    \node[auth] (solver) at (6.7,0.4) {Pyomo + HiGHS};
    \node[ok] at (6.7,-0.25) {AUTHORITY};
    ...
    \draw[ar] (graph.south) -- (policy.north);
...
\begin{frame}
  ...
  \begin{tikzpicture}[
    ...
    \node[core] (cand)  at (0,1.3)  {LoRA candidate};
    ...
    \node[lbl,left=1.5mm of cand] {$\sim$1.7S};
...
\begin{frame}
  ...
  \begin{itemize}\setlength\itemsep{0.75em}
    \item[\color{arCyan}$\bullet$] \gn{94.4\% feasible replans} at the size it was trained for
