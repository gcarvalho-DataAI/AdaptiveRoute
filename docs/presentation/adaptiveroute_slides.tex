...
\setbeamertemplate{footline}{%
  \hbox{\begin{beamercolorbox}[wd=\paperwidth,ht=2.6ex,dp=1.6ex,leftskip=1.5em,rightskip=1.5em]{}%
    {\color{arFaint}\scriptsize\bfseries AdaptiveRoute}%
    \hfill{\color{arMuted}\scriptsize\bfseries\insertframenumber\,/\,19}%
  \end{beamercolorbox}}%
}
...
\begin{frame}
  ...
  \vspace{1.0em}
  \centering
  \gn{No exceptions.} The same gate applies to the model, the repair and the solver.

  \note{Two properties make this more than a checklist. First, the validator recomputes load and
  distance from the scenario --- the model supplies only the visit sequence, so a hallucinated number
  cannot survive. Second, failure is a prompt rather than an error: the specific violations
  (``capacity\_violation: load 45, capacity 40'') are fed back as a structured correction prompt,
  three attempts, then the solver takes over.}
\end{frame}

% ================================================================ 8 conversational layer
\begin{frame}
  \frametitle{What makes the conversation work}
  \framesubtitle{AdaptiveRoute \textperiodcentered\ The conversational layer}
  ...
  \begin{columns}[T]
    \begin{column}{0.315\linewidth}
      \begin{panel}{\linewidth}
        \plabel{Memory}\\[0.5em]
        \ptitle{Context window}\\[0.45em]
        \muted{\small A bounded summary per conversation: last event, last plan, open
        constraints.}\\[0.6em]
        \faint{The driver never repeats themselves.}
      \end{panel}
    \end{column}
    \begin{column}{0.315\linewidth}
      \begin{panel}{\linewidth}
        \plabel{Retrieval}\\[0.5em]
        \ptitle{Documentation in pgvector}\\[0.45em]
        \muted{\small Project docs chunked and embedded; cosine search returns supporting
        passages.}\\[0.6em]
        \faint{Answers cite sources, not recall.}
      \end{panel}
    \end{column}
    \begin{column}{0.315\linewidth}
      \begin{panelglow}{\linewidth}
        \plabel{Grounding}\\[0.5em]
        \ptitle{Facts before the model}\\[0.45em]
        \muted{\small Route facts are computed from the stored plan, then handed to the
        model.}\\[0.6em]
        \faint{The answer cannot drift from the plan.}
      \end{panelglow}
    \end{column}
  \end{columns}
  ...
  \vspace{1.1em}
  \centering
  \small Questions are answered from the stored plan.
  \cy{Only disruptions enter the replanning workflow.}

  \note{This is the layer that makes a chat interface viable rather than a demo. The context window
  is rebuilt after every turn: previous summary carried forward, extended with the new event, the
  validation outcome and the replanning impact, then truncated to a budget. Retrieval indexes the
  project documentation into Postgres with pgvector and searches by cosine distance. And the Q\&A
  pipeline computes route facts from persisted data before calling the model, so the answer is
  anchored to the actual plan. The distinction at the bottom matters: a question is answered from
  stored state, but anything implying a route change is routed into the workflow where the
...
\begin{frame}
  ...
  \begin{enumerate}\setlength\itemsep{0.55em}
    \item[\color{arCyan}\bfseries 1.] Generate a synthetic CVRP instance
    \item[\color{arCyan}\bfseries 2.] Solve it to optimality with HiGHS
    \item[\color{arCyan}\bfseries 3.] Inject an operational event
    \item[\color{arCyan}\bfseries 4.] \cy{Re-solve} the disrupted scenario
    \item[\color{arCyan}\bfseries 5.] Validate, then serialise the pair
  \end{enumerate}
  \normalsize

  \vspace{1.6em}
  \centering
  \muted{The solver labels its own training set.}

  \note{The model learns to imitate optimal replanning, not to invent it. Only the routing policy is
  fine-tuned --- the orchestrator stays off the shelf. Full configuration is in the appendix: LoRA
  rank 16, alpha 32, dropout 0.05, 4-bit NF4, bfloat16, one epoch, on a single RTX 3090.}
\end{frame}
...
\begin{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.49\linewidth}
      \begin{panelglow}{\linewidth}
        ...
        {\color{arText}\Large\bfseries 8--9 customers}\\[0.7em]
        \muted{\small Above it the model omits customers --- it does not route them badly.}\\[0.9em]
        \cy{A cardinality limit, not a routing one.}
      \end{panelglow}
    \end{column}
  \end{columns}
  ...
  it. The fix is training data at 10--24 customers. And note that beyond the boundary the plan is
  still correct: the cascade simply falls through to the solver every time. Samples are small ---
  3--5 scenarios per size.}
\end{frame}
...
\begin{frame}
  ...
  \framesubtitle{AdaptiveRoute \textperiodcentered\ What this establishes}
  ...
  \normalsize
  \centering
  \faint{The interesting result is not the model --- it is everything built around it.}
...
% ================================================================ 17 theory
\newcommand{\refitem}[3]{%
  {\color{arText}\footnotesize\bfseries #1}\;{\color{arFaint}\footnotesize #2}\\[0.1em]
  {\color{arMuted}\scriptsize #3}\\[0.55em]}

\begin{frame}
  \frametitle{Where this sits in the literature}
  \framesubtitle{AdaptiveRoute \textperiodcentered\ Theoretical grounding}

  \vspace{0.6em}
  \begin{columns}[T]
    \begin{column}{0.315\linewidth}
      \begin{panel}{\linewidth}
        \plabel{Exact routing}\\[0.7em]
        \refitem{Dantzig, Ramser}{1959}{The truck dispatching problem. \emph{Management Science} 6(1).}
        \refitem{Miller, Tucker, Zemlin}{1960}{Integer programming formulation of TSP. \emph{J. ACM} 7(4).}
        \refitem{Huangfu, Hall}{2018}{Parallelizing the dual revised simplex method. \emph{Math. Prog. Comp.} 10(1).}
      \end{panel}
    \end{column}
    \begin{column}{0.315\linewidth}
      \begin{panel}{\linewidth}
        \plabel{Learning for optimisation}\\[0.7em]
        \refitem{Vinyals, Fortunato, Jaitly}{2015}{Pointer networks. \emph{NeurIPS}.}
        \refitem{Kool, van Hoof, Welling}{2019}{Attention, learn to solve routing problems! \emph{ICLR}.}
        \refitem{Bengio, Lodi, Prouvost}{2021}{ML for combinatorial optimization: a tour d'horizon. \emph{EJOR} 290(2).}
      \end{panel}
    \end{column}
    \begin{column}{0.315\linewidth}
      \begin{panelglow}{\linewidth}
        \plabel{Efficient LLM adaptation}\\[0.7em]
        \refitem{Hu et al.}{2021}{LoRA: low-rank adaptation of large language models. arXiv:2106.09685.}
        \refitem{Dettmers et al.}{2023}{QLoRA: efficient finetuning of quantized LLMs. arXiv:2305.14314.}
      \end{panelglow}
    \end{column}
  \end{columns}
  ...
  \vspace{0.8em}
  \centering
  \small Prior work replaces the solver. \cy{This keeps it as the authority.}

  \note{The positioning matters. Pointer Networks and Kool et al. ask whether a neural network can
  solve routing end to end --- impressive, but no feasibility guarantee. Bengio, Lodi and Prouvost
  survey the hybrid alternative, where learning assists rather than replaces exact methods. This
  project takes that hybrid stance and pushes it further: the learned component is a fine-tuned LLM
  operating on natural language rather than a graph neural network, and the exact solver remains the
  final authority on every plan.}
\end{frame}

% ================================================================ 18 appendix
\begin{frame}
  \frametitle{Training configuration}
  \framesubtitle{AdaptiveRoute \textperiodcentered\ Appendix}
    ...
    \begin{column}{0.36\linewidth}
      \begin{panel}{\linewidth}
        ...
        \begin{tabular}{@{}l@{\hspace{1.2em}}r@{}}
          ...
          Epochs        & \color{arText}\bfseries 1 \\[0.5em]
        \end{tabular}
      \end{panel}
    \end{column}
    \begin{column}{0.52\linewidth}
      \begin{panel}{\linewidth}
        \plabel{Runs \textemdash\ each continues the previous adapter}\\[0.8em]
        \color{arMuted}\footnotesize
        \begin{tabular}{@{}l@{\hspace{1.0em}}r@{\hspace{1.0em}}r@{\hspace{1.0em}}r@{\hspace{1.0em}}r@{}}
          & \color{arCyan}\scriptsize\bfseries ROWS & \color{arCyan}\scriptsize\bfseries STEPS
          & \color{arCyan}\scriptsize\bfseries LR & \color{arCyan}\scriptsize\bfseries TIME \\[0.25em]
          \color{arText}v3 & 24{,}000 & 3{,}000 & 5e-5   & 5h53m \\
          \color{arText}v4 & 56{,}000 & 2{,}300 & 1.7e-5 & 3h44m \\
          \rowcolor{arPanel2}
          \color{arCyan2}\bfseries v5 & \color{arText}\bfseries 16{,}000 &
            \color{arText}\bfseries 1{,}500 & \color{arText}\bfseries 1e-5 &
            \color{arText}\bfseries 2h22m \\
          \color{arText}v6 &  8{,}000 &   800 & 7e-6   & 1h18m \\[0.4em]
          \color{arMuted}total & & & & \color{arText}\bfseries $\sim$13h \\
        \end{tabular}\\[0.9em]
        \faint{One epoch per run, batch 8, cosine schedule, RTX 3090.}
      \end{panel}
    \end{column}
  \end{columns}
